<script>
  ...
  const loadProductByCategory = async () => {
      const res = await getKLoukdoProductsByCategory.load({category:categoryId , limit:14});
      products = res.data;
...
<div class="flex flex-col mx-auto w-96 border overflow-y-scroll"
  on:scroll={updateY}
>
  <!-- Header with Search -->
  <KLoukdoHeader 
  ...
  <!-- Filter by Brand -->
   <div>
    <div class="flex gap-3 overflow-x-scroll w-full p-2">
      {#each SubCategories as subCategory}
        <div class=" bg-white p-2 rounded-md text-center h-20 shrink-0">
          {#if subCategory.icon}
